...
/**
 * Repara un cromosoma para que tenga exactamente teamSize bits 1.
 * Si hay demasiados 1s, elimina algunos al azar.
 * Si hay muy pocos 1s, agrega algunos al azar.
 * @param {Array<0|1>} chromo
 * @param {number} teamSize — número objetivo de bits 1
 * @returns {Array<0|1>}
 */
function repairChromosome(chromo, teamSize) {
  const ones = chromo.map((b, i) => b === 1 ? i : -1).filter(i => i >= 0);
  const zeros = chromo.map((b, i) => b === 0 ? i : -1).filter(i => i >= 0);
  const current = ones.length;

  if (current === teamSize) return chromo;

  const repaired = [...chromo];

  if (current > teamSize) {
    // Eliminar bits 1 extras al azar
    const toRemove = current - teamSize;
    const indicesToRemove = shuffleArray([...ones]).slice(0, toRemove);
    indicesToRemove.forEach(i => { repaired[i] = 0; });
  } else {
    // Agregar bits 1 faltantes al azar
    const toAdd = teamSize - current;
    const indicesToAdd = shuffleArray([...zeros]).slice(0, toAdd);
    indicesToAdd.forEach(i => { repaired[i] = 1; });
  }
 ...
 * Repara los hijos para asegurar exactamente teamSize bits 1.
 * @param {Array<0|1>} parent1
 * @param {Array<0|1>} parent2
 * @param {number} crossRate — probabilidad de cruce (0-1)
 * @param {number} teamSize — número objetivo de bits 1 en el cromosoma
 * @returns {{child1: Array, child2: Array, point: number}}
 */
function onePointCrossover(parent1, parent2, crossRate, teamSize) {
  ...
  let child1 = [...parent1.slice(0, point), ...parent2.slice(point)];
  let child2 = [...parent2.slice(0, point), ...parent1.slice(point)];
  
  // Reparar ambos hijos para mantener el tamaño del equipo
  child1 = repairChromosome(child1, teamSize);
  child2 = repairChromosome(child2, teamSize);
...
function runGA(params) {
  ...
  for (let gen = 1; gen <= maxGen; gen++) {
    ...
    while (newPop.length < popSize) {
      ...
      // Crossover
      const { child1, child2 } = onePointCrossover(p1.chromo, p2.chromo, cRate, teamSize);
